...
dvec2 Integrator::RK4(const VectorField2& vectorField, const dvec2& position,
    ...
    dvec2 movement;
    if (forwards) movement = dvec2(finalDirection.x * stepSize, finalDirection.y * stepSize);
    else movement = dvec2(finalDirection.x * stepSize * -1, finalDirection.y * stepSize * -1);
