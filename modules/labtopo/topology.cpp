...
int calcBorderSwitchPoints(dvec2 start, dvec2 end, function<bool(dvec2)> isPointingInside,
                           function<bool(dvec2, dvec2)> foundBorder, int maxIterations,
                           function<void(dvec2, vec4)> point,
                           function<void(dvec2, bool)> drawStreamline) {
    if (maxIterations <= 0) {
    ...
    if (foundBorder(start, center) || foundBorder(center, end)) {
        //draw
        point(center, Topology::ColorsCP[(int)Topology::TypeCP::Border]);
        drawStreamline(start, true);
        drawStreamline(start, false);
    ...
    if (isPointingInside(start) != isPointingInside(center)) {
        // start to center
        return calcBorderSwitchPoints(start, center, isPointingInside, foundBorder,
                                      maxIterations - 1, point, drawStreamline);
    } else if (isPointingInside(center) != isPointingInside(end)) {
        return calcBorderSwitchPoints(center, end, isPointingInside, foundBorder, maxIterations - 1,
                                      point, drawStreamline);
    }
    return maxIterations;
};
...
void Topology::process() {
    ...
    auto isInside = [&](dvec2 vector) {
        double offset = -FLT_EPSILON * 1000;
        return BBoxMax.x - vector.x > offset &&
               BBoxMax.y - vector.y > offset &&
               vector.x - BBoxMin.x > offset &&
               vector.y - BBoxMin.y > offset;
    };
    ...
    auto calcStreamline = [&](vec2 startPoint, bool forward) {
        ...
        for (; i < stepsProp; i++) {
            ...
            arcLength += distance;
            drawLineSegment(currentPoint, newPoint, vec4(1, 1, 1, 1), indexBufferSeparatrices.get(),
                            vertices);
    ...
    auto xIsZero = [&](dvec2 start, dvec2 end) {
        auto startX = vectorField.interpolate(start).x;
        auto endX = vectorField.interpolate(end).x;
        return abs(endX - startX) < FLT_EPSILON || abs(startX) < FLT_EPSILON || abs(endX) <
               FLT_EPSILON;
    };
    auto yIsZero = [&](dvec2 start, dvec2 end) {
        auto startY = vectorField.interpolate(start).y;
        auto endY = vectorField.interpolate(end).y;
        return abs(endY - startY) < FLT_EPSILON || abs(startY) < FLT_EPSILON || abs(endY) <
               FLT_EPSILON;
    };
    ...
    for (double x = topLeft.x; x <= topRight.x; x += xDiff / amountOfSteps) {
        ...
        if (lastPointingInside != pointingInside) {
            calcBorderSwitchPoints(lastLocation, location, isPointingUp, xIsZero, maxIterations,
                                   point, calcStreamline);
        }
    ...
    for (double x = topLeft.x; x <= topRight.x; x += xDiff / amountOfSteps) {
        ...
        if (lastPointingInside != pointingInside) {
            ...
            calcBorderSwitchPoints(lastLocation, location, isPointingDown, xIsZero, maxIterations,
                                   point, calcStreamline);
            // LogProcessorWarn("Border switch point added in " << maxIterations - num << " Iterations")
        }
        lastPointingInside = pointingInside;
        lastLocation = location;
    }
    ...
    for (double y = bottomLeft.y; y <= topLeft.y; y += yDiff / amountOfSteps) {
        ...
        if (lastPointingInside != pointingInside) {
            ...
            // point(location, ColorsCP[(int) Topology::TypeCP::Border] );
            calcBorderSwitchPoints(lastLocation, location, isPointingRight, yIsZero, maxIterations,
                                   point, calcStreamline);
            // LogProcessorWarn("Border switch point added in " << maxIterations - num << " Iterations")
        }
        lastPointingInside = pointingInside;
    ...
    for (double y = bottomRight.y; y <= topRight.y; y += yDiff / amountOfSteps) {
        ...
        if (lastPointingInside != pointingInside) {
            ...
            // point(location, ColorsCP[(int) Topology::TypeCP::Border] );
            calcBorderSwitchPoints(lastLocation, location, isPointingLeft, yIsZero, maxIterations,
                                   point, calcStreamline);
        }
        lastPointingInside = pointingInside;
        lastLocation = location;
